    ...
    >
      ...
      {previewUrl ? (
        <div className="relative w-full flex flex-col items-center gap-3">
          ...
          />
          <span className="text-[14px] text-[#868889]">{file?.name ?? "Current thumbnail — click to replace"}</span>
        </div>
      ) : (
        <>
...
export default function EditCourseOverviewPage() {
  ...
  const { data: editorData, isLoading: isLoadingCourse } = useQuery({
  ...
  if (isLoadingCourse) {
    return (
      <div className="flex-1 flex items-center justify-center min-h-screen" style={{ background: "#121415" }}>
        <span className="text-[#868889] text-[16px]">Loading course...</span>
      </div>
    );
  }
