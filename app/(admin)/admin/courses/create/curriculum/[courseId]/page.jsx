            ...
            <div className="flex flex-col gap-2 px-3 py-3">
              ...
              <div className="flex items-center gap-2">
                ...
                {lesson.videoEmbed && (
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button className="text-[#ABADAF] hover:text-[#DFE1E3] transition-colors"><IconEdit /></button>
                    <button className="text-[#ABADAF] hover:text-[#DFE1E3] transition-colors"><IconEye /></button>
                    <button onClick={() => onUpdate({ videoEmbed: "" })} className="text-[#C0696B] hover:text-[#E07678] transition-colors">
                      <IconTrash />
                    </button>
                  </div>
                )}
              ...
              {(lesson.resources || []).map((res) => (
                <ResourceRow
                  key={res._id}
                  resource={res}
                  onDelete={() => onUpdate({ resources: lesson.resources.filter((r) => r._id !== res._id) })}
                />
              ))}
          ...
          <div className="flex flex-col gap-3" style={{ paddingLeft: 70 }}>
            {module.lessons.map((lesson, li) => (
              <LessonCard
                ...
                onDelete={() => onUpdate({ lessons: module.lessons.filter((l) => l._id !== lesson._id) })}
              />
            ))}
          </div>
...
async function syncCurriculum({ courseId, modules, deletedModuleIds, deletedLessonMap }) {
  // 1. Delete removed modules (real ones only)
  for (const moduleId of deletedModuleIds) {
    await deleteAdminModule({ courseId, moduleId });
  }

  // 2. Delete removed lessons (real ones only)
  for (const [moduleId, lessonIds] of Object.entries(deletedLessonMap)) {
    for (const lessonId of lessonIds) {
      await deleteAdminLesson({ courseId, moduleId, lessonId });
    }
  }

  // 3. Create / update modules and their lessons in order
...
export default function CurriculumPage() {
  ...
  // Track real IDs that were deleted (so we can call delete API on save)
  const [deletedModuleIds, setDeletedModuleIds] = useState([]);
  const [deletedLessonMap, setDeletedLessonMap] = useState({}); // { moduleId: [lessonId, ...] }
  ...
  // Initialize local state from API (once)
  useEffect(() => {
  ...
  const handleSave = async (andNavigate) => {
    ...
    setIsSaving(true);
    try {
      await syncCurriculum({ courseId, modules, deletedModuleIds, deletedLessonMap });
      // Clear deleted trackers after successful save
      setDeletedModuleIds([]);
      setDeletedLessonMap({});
      // Mark all items as real (refetch editor to get real IDs)
      setInitialized(false);
      if (andNavigate) {
        router.push(`/admin/courses/create/preview/${courseId}`);
      }
    } catch (err) {
